...
class KrakenConnector:
    ...
    def _calculate_usd_value(
        self, currency: str, amount: float, tickers: dict
    ) -> Decimal:
        """
        Calculate USD value for a given currency and amount
    ...
    def print_trades(self, trades: list, detailed: bool = False):
        ...
        if detailed:
            ...
            # Summary table view
            print(
                f"{'Date':<20} {'Symbol':<12} {'Side':<6} {'Amount':>15} {'Price':>12} {'Cost':>12}"
            )
            print(f"{'-'*80}")

            for trade in trades:
                date_str = (
                    trade.get("datetime", "N/A")[:19]
                ...
                print(
                    f"{date_str:<20} {symbol:<12} {side_display:<6} {amount:>15,.8f} ${price:>11,.2f} ${cost:>11,.2f}"
                )
